...
namespace Carm.Tg
{
    ...
    // Sistema                   : Sima
    // Interface para la Entidad : TipoInst
    ...
    /// <summary>
    /// Formulario para Mantenimiento de la Tabla:TipoInst
    /// </summary>
    public partial class TipoInsts : DockContent
    {
        #region Miembros de la Clase
            private Bel.ETipoInst m_entTipoInst= null;
        ...
        /// </summary>
        private void TipoInsts_Load(object sender, System.EventArgs e)
        {
            // Inicializamos el form
            App.ShowMsg("Inicializando el formulario...");

            // Llenamos las Combos (por Lista)
            cmbMayorista.AddStrCD("S", " SI", 0);
            cmbMayorista.AddStrCD("N", " NO", 0);

            // Llenamos las Combos (por Tablas)
            Bel.LERubros l_lentRubros= Bll.Tablas.RbrUpFull(false, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
            cmbCodrubro.FillFromStrLEntidad(l_lentRubros, "rbr_rcd_cod", "rbr_des_des", "deleted");
        ...
        /// </summary>
        private void TipoInsts_FormClosed(object sender, FormClosedEventArgs e)
        {
            // Liberamos el menu
            App.LockMenu(false);
        }
        ...
        private void grdDatos_HeaderClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (m_strSort != grdDatos.GridOrder) {
                    ...
                    App.SetStrURegistry(false, "GridFormat", "TipoInstsGrdSort", m_strSort);
                    return;
                }
            }
        ...
        private void GrdColumn_WidthChanged(object sender, EventArgs e)
        {
            ...
            App.SetStrURegistry(false, "GridFormat", "TipoInstsGrdWidths", grdDatos.ColWitdhs);
        }
        ...
        private void cmdExcel_Click(object sender, System.EventArgs e)
        {
            ...
            grdDatos.ExportToExcel(false, false, "", "TipoInsts", m_smResult);
            App.EndAdvance();
            App.HideMsg();
        }
        ...
        private void gbNuevo_Click(object sender, System.EventArgs e)
        {
            ...
            // damos foco al primer campo
            m_entTipoInst= Bel.ETipoInst.NewEmpty();
        ...
        private void gbModificar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entTipoInst= Bll.Tablas.TinGet((string) grdDatos.GetMatrixValueObj(l_iRow, 1),
                                             false, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void gbCompactar_Click(object sender, EventArgs e)
        {
            ...
            if (MsgRuts.AskUser(this, "Atención!!!!\r\n" +
                                      ...
                                      /*App.Usuario.Usuario +*/ "TipoInstsPurge") == DialogResult.No) return;

            // Purgamos la tabla
            App.ShowMsg("Compactando la tabla...");
            Bll.Tablas.TinPurge(m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void gbChangeHabilitado_Click(object sender, System.EventArgs e)
        {
            ...
            Bll.Tablas.TinEnabled(m_entTipoInst.EstaBorrada,
                                  m_entTipoInst.Cod,
                                  m_entTipoInst.FxdVersion,
                                  m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void gbGrabar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entTipoInst.Cod= txtCod.Text;
            m_entTipoInst.Des= txtDes.Text;
            m_entTipoInst.Codrubro= cmbCodrubro.SelectedStrCode;
            m_entTipoInst.Mayorista= cmbMayorista.SelectedStrCode;

            // Tratamos de grabar la entidad
            App.ShowMsg("Grabando...");
            Bll.Tablas.TinSave(m_entTipoInst, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void FillGrid()
        {
            ...
            App.ShowMsg("Recuperando datos...");
            Bel.LETipoInsts l_lentTipoInsts= Bll.Tablas.TinUpFull(false, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;

            // Asignamos a la grilla
            App.InitAdvance("Cargando:");
            grdDatos.Focus();
            grdDatos.FillFromLEntidad(l_lentTipoInsts, "deleted");
            grdDatos.ColWitdhs= App.GetStrURegistry(false, "GridFormat", "TipoInstsGrdWidths", "");
            grdDatos.GridOrder= App.GetStrURegistry(false, "GridFormat", "TipoInstsGrdSort", "");
            App.EndAdvance();

            // Fijamos el evento de cambio de ancho de la grilla
            if (l_lentTipoInsts.Count > 0)
        ...
        private void OperationMode()
        {
            ...
            cmbCodrubro.NormalDisable= true;
            cmbCodrubro.Enabled= false;
            cmbMayorista.NormalDisable= true;
            cmbMayorista.Enabled= false;
            ...
            txtCod.Text= "";
            txtDes.Text= "";
            cmbCodrubro.SelectedStrCode= "";
            cmbMayorista.SelectedStrCode= "";
        ...
        private void EditMode()
        {
            ...
            txtCod.Text= m_entTipoInst.Cod;
            txtDes.Text= m_entTipoInst.Des;
            cmbCodrubro.SelectedStrCode= m_entTipoInst.Codrubro;
            cmbMayorista.SelectedStrCode= m_entTipoInst.Mayorista;

            // Habilitamos el frame
            txtCod.NormalDisable= false;
            txtCod.Enabled= m_entTipoInst.EsNueva;
            txtDes.NormalDisable= false;
            txtDes.Enabled= m_entTipoInst.EsNueva;
            cmbCodrubro.NormalDisable= false;
            cmbCodrubro.Enabled= !m_entTipoInst.EstaBorrada;
            cmbMayorista.NormalDisable= false;
            cmbMayorista.Enabled= !m_entTipoInst.EstaBorrada;
            gbCancelar.Enabled= true;
            gbGrabar.Enabled= !m_entTipoInst.EstaBorrada;
            gbDeshabilitar.Enabled= ((!m_entTipoInst.EsNueva) &&(!m_entTipoInst.EstaBorrada));
