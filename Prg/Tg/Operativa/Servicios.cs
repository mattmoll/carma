...
namespace Carm.Tg
{
    ...
    //----------------------------------------------------------------------------
    // Fecha                     : 07/08/2020 00:01
    // Sistema                   : Carm
    // Interface para la Entidad : Servicio
    ...
    /// <summary>
    /// Formulario para Mantenimiento de la Tabla:Servicios
    /// </summary>
    public partial class Servicios : DockContent
    {
        #region Miembros de la Clase
            private Bel.EServicio m_entServicio= null;
        ...
        private void grdDatos_HeaderClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left) {
                ...
                if (m_strSort != grdDatos.GridOrder) {
                    ...
                    App.SetStrURegistry(false, "GridFormat", "ServiciosGrdSort", m_strSort);
                    return;
                }
            }
        ...
        private void GrdColumn_WidthChanged(object sender, EventArgs e)
        {
            ...
            App.SetStrURegistry(false, "GridFormat", "ServiciosGrdWidths", grdDatos.ColWitdhs);
        }
        ...
        private void cmdExcel_Click(object sender, System.EventArgs e)
        {
            ...
            grdDatos.ExportToExcel(false, false, "", "Servicios", m_smResult);
            App.EndAdvance();
            App.HideMsg();
        }
        ...
        private void cmdNuevo_Click(object sender, System.EventArgs e)
        {
            ...
            // damos foco al primer campo
            m_entServicio= Bel.EServicio.NewEmpty();
        ...
        private void cmdModificar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entServicio= Bll.Servicios.Get((string) grdDatos.GetMatrixValueObj(l_iRow, 1),
                                             false, m_smResult);
        ...
        private void cmdPurgar_Click(object sender, EventArgs e)
        {
            ...
            if (MsgRuts.AskUser(this, "Atención!!!!\r\n" +
                                      ...
                                      /*App.Usuario.Usuario +*/ "ServiciosPurge") == DialogResult.No) return;

            // Purgamos la tabla
            App.ShowMsg("Compactando la tabla...");
            Bll.Servicios.Purge(m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void cmdDesHab_Click(object sender, System.EventArgs e)
        {
            ...
            Bll.Servicios.Enabled(m_entServicio.EstaBorrada,
                                  m_entServicio.Cod,
                                  m_entServicio.FxdVersion,
                                  m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void cmdGrabar_Click(object sender, System.EventArgs e)
        {
            ...
            m_entServicio.Cod= txtCod.Text;
            m_entServicio.Des= txtDes.Text;
            m_entServicio.Explicacion= txtExplicacion.Text;

            // Tratamos de grabar la entidad
            App.ShowMsg("Grabando...");
            Bll.Servicios.Save(m_entServicio, m_smResult);
            if (MsgRuts.AnalizeError(this, m_smResult)) return;
        ...
        private void FillGrid()
        {
            ...
            grdDatos.Focus();
            grdDatos.FillFromLEntidad(l_lentServicios, "deleted");
            grdDatos.ColWitdhs= App.GetStrURegistry(false, "GridFormat", "ServiciosGrdWidths", "");
            grdDatos.GridOrder= App.GetStrURegistry(false, "GridFormat", "ServiciosGrdSort", "");
            App.EndAdvance();

            // Fijamos el evento de cambio de ancho de la grilla
            if (l_lentServicios.Count > 0)
        ...
        private void OperationMode()
        {
            ...
            txtExplicacion.NormalDisable= true;
            txtExplicacion.Enabled= false;
        ...
        private void EditMode()
        {
            ...
            txtCod.Text= m_entServicio.Cod;
            txtDes.Text= m_entServicio.Des;
            txtExplicacion.Text= m_entServicio.Explicacion;

            // Habilitamos el frame
            txtCod.NormalDisable= false;
            txtCod.Enabled= m_entServicio.EsNueva;
            txtDes.NormalDisable= false;
            txtDes.Enabled= m_entServicio.EsNueva;
            txtExplicacion.NormalDisable= false;
            txtExplicacion.Enabled= !m_entServicio.EstaBorrada;
            cmdCancelar.Enabled= true;
            cmdGrabar.Enabled= !m_entServicio.EstaBorrada;
            cmdDesHab.Enabled= ((!m_entServicio.EsNueva) &&(!m_entServicio.EstaBorrada));
